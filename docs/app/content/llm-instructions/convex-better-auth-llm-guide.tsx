...
export type Framework = "react" | "nextjs" | "tanstack";

interface ConvexBetterAuthLLMGuideProps {
  framework: Framework;
  title?: string;
}

export default function ConvexBetterAuthLLMGuide({ 
  framework, 
  title 
}: ConvexBetterAuthLLMGuideProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  const guideContent = getGuideContent(framework);
  const stepCount = guideContent.split('\n').filter((line: string) => line.startsWith('# ')).length;
  ...
  const displayTitle = title || `LLM Instructions: Convex Better Auth Setup (${frameworkDisplayName})`;
  
  const previewContent = `### These are steps to setup better auth with convex and ${framework}
      ...
      <div className="border rounded-lg overflow-hidden">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 px-4 py-3 border-b flex items-center justify-between">
          ...
          >
            ...
            {isExpanded ? 'Hide Guide' : `Show Complete Guide (${stepCount} steps)`}
          </button>
        ...
        {!isExpanded && (
          <div className="bg-gray-50 dark:bg-gray-800 px-4 py-3 border-t">
            ...
            >
              Click to view all {stepCount} steps →
            </button>
          </div>
        )}
      </div>
...
function getGuideContent(framework: Framework): string {
  switch (framework) {
    case "nextjs":
...
npm install convex@latest
\`\`\`
# 2. Setup React specific configuration
[Add React-specific setup instructions here]`;
    case "tanstack":
      return `### These are steps to setup better auth with convex and tanstack
# 1. Install the dependencies
\`\`\`bash
npm install @convex-dev/better-auth
npm install better-auth@1.2.12 --save-exact
npm install convex@latest
\`\`\`
# 2. Setup TanStack specific configuration
